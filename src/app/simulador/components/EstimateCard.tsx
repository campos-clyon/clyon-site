...
function buildWhatsAppMessage(order: OrderData, estimate: EstimateResult | null): string {
  const lines = [
    "Olá CLYON, quero confirmar este pedido:",
    ...
    `Andar: ${order.floor ?? "—"}`,
    `Elevador: ${order.hasElevator ?? "—"}`,
    `Estacionamento: ${order.parkingDistance ?? "—"}`,
    `Resumo: ${order.description ?? "—"}`,
    estimate?.status === "estimated"
      ? `Estimativa: ${estimate.estimatedPriceWithoutVat}€ + IVA (total ~${estimate.estimatedPriceWithVat}€)`
      : "Estimativa: a calcular",
  ];
